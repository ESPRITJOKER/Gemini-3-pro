...
const ScoreBadge = ({ label, value, type }: { label: string, value: number, type: 'risk' | 'good' | 'neutral' }) => {
  ...
  if (type === 'risk') {
    // High risk is bad (Red), low risk is good (Green)
    colorClass = value > 60 ? 'text-oracle-risk border-oracle-risk' : 'text-oracle-safe border-oracle-safe';
  } else if (type === 'good') {
    // High is good (Green), Low is caution (Yellow)
    colorClass = value > 60 ? 'text-oracle-safe border-oracle-safe' : 'text-yellow-500 border-yellow-500';
  } else {
    // Neutral/Cost - High cost (Orange), Low cost (Blue)
    colorClass = value > 60 ? 'text-orange-400 border-orange-400' : 'text-blue-400 border-blue-400';
  }

  return (
    <div className={`flex flex-col items-center justify-center p-3 rounded-lg border ${colorClass} bg-transparent flex-1 min-w-[80px] transition-all hover:bg-white/5`}>
      <span className="text-[10px] font-mono uppercase font-bold opacity-90 tracking-wider text-center mb-1">{label}</span>
...
const OptionCard: React.FC<OptionCardProps> = ({ option, isRecommended }) => {
  return (
    <div className={`relative p-6 rounded-lg border transition-all duration-300 hover:scale-[1.01] flex flex-col h-full ${isRecommended ? 'bg-oracle-800 border-oracle-accent shadow-[0_0_15px_rgba(0,240,255,0.2)]' : 'bg-oracle-800/50 border-oracle-700 hover:border-oracle-500'}`}>
      {isRecommended && (
        <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 bg-oracle-accent text-oracle-900 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider flex items-center gap-1 shadow-lg z-10">
      ...
      <p className="text-gray-400 text-sm mb-6 leading-relaxed flex-grow">{option.description}</p>
